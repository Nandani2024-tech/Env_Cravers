...
def _free_resource(resource_id: str, env_state: EnvState, patient_observations: Optional[dict] = None) -> Optional[str]:
    ...
    if patient_id is not None:
        ...
        # Clear assigned_resource in patient observation to maintain state coherence
        if patient_observations and patient_id in patient_observations:
            current_obs = patient_observations[patient_id]
            patient_observations[patient_id] = current_obs.model_copy(
                update={"assigned_resource": None}
            )
    return patient_id

def process_step_releases(env_state: EnvState, patient_observations: Optional[dict] = None) -> list[str]:
...
def execute_early_discharge(patient_id: str, env_state: EnvState, patient_observations: Optional[dict] = None) -> tuple[bool, Optional[str]]:
    ...
    for rid in assigned_resources:
        _free_resource(rid, env_state, patient_observations)
